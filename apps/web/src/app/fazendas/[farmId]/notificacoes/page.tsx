    ...
    <div className="animate-fade-up mx-auto w-full max-w-6xl px-4 py-8 sm:px-8">
        ...
        actions={
          <div className="flex gap-2">
            <button
              ...
              disabled={generating}
              className="rounded-full bg-gray-900/5 px-5 py-2.5 text-sm font-semibold text-gray-800 transition-colors duration-150 hover:bg-gray-900/10 disabled:opacity-50"
            >
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        {selected.size > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-full bg-red-600 px-4 py-2 text-sm font-semibold text-white transition-colors duration-150 hover:bg-red-700"
          >
            <Trash2 size={14} />
      ...
      {notifications.length === 0 ? (
        <div className="flex flex-col items-center rounded-2xl bg-gray-100/60 px-6 py-14 text-center">
          <span className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-emerald-600/10 text-emerald-700">
            <Bell size={22} />
          </span>
          <p className="text-lg font-bold text-gray-900">Nenhuma notificação</p>
        ...
        <>
          ...
          <ul className="space-y-2">
            {notifications.map((n) => (
              <li
                key={n.id}
                className={`flex gap-3 rounded-2xl border px-4 py-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_10px_30px_-12px_rgba(6,30,20,0.15)] ${
                  selected.has(n.id)
                ...
                <div className="flex flex-1 items-start justify-between gap-4">
                  ...
                  {!n.read && (
                    <button
                      ...
                      className="shrink-0 rounded-full bg-gray-900/5 px-3 py-1 text-xs font-semibold text-gray-800 transition-colors duration-150 hover:bg-gray-900/10"
                    >
                      Marcar como lida
                    </button>
                  )}
